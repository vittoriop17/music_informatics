...
def dataset_preprocessor(input_path, normalize_amplitude, spectrogram, normalize_features = False, class_set = None, output_path = 'C:\\Users\\Prestige\\Desktop\\Paolo\\UNi\\ERASMUS\\KTH\\P1\\Music Informatics\\fp_musinfo\\music_informatics\\data'):
    ...
    for instr in instr_folder:
        ...
        if instr in class_set:
            for audio_name in os.listdir(instr_path):
                ...
                data_labels[index] = return_label(instr)         # save the label of the sample
    ...
    np.save(os.path.join(output_path,'out_dataset_test.npy'), data)

    print('Done.')

    print('saving labels to .npy file...')
    np.save(os.path.join(output_path, 'out_labels_test.npy'), data_labels)

    print('Done.')

    return data, data_labels

def test_dataset_preprocessor(input_path, normalize_amplitude, spectrogram, normalize_features = False, class_set = None, output_path = 'C:\\Users\\Prestige\\Desktop\\Paolo\\UNi\\ERASMUS\\KTH\\P1\\Music Informatics\\fp_musinfo\\music_informatics\\data'):
    '''
    ...
    for instr in instr_folder:
        ...
        if instr in class_set:
            for audio_name in os.listdir(instr_path):
                ...
                if len(samples_audio) > 132299:        # if the audio is more then 3 second
                    samples_audio = samples_audio[:132299]
                    print('resized audio, was too long')
                if len(samples_audio) < 132299:
                    new_samples_audio = np.zeros(132299)
                    new_samples_audio[:len(samples_audio)] = samples_audio
                    samples_audio = new_samples_audio
                    print('resized audio, was too short')
...
def make_hist_cardinailty(input_path):
    class_list = ['cel', 'cla', 'flu', 'gac', 'gel', 'org', 'pia', 'sax', 'tru', 'vio', 'voi']
    class_cardinality = {}
    ...
    for instr in instr_folder:
        instr_path = os.path.join(input_path, instr)
        class_cardinality[instr] = len(os.listdir(instr_path))

    plt.bar(class_cardinality.keys(), class_cardinality.values(), color='orange')
    plt.ylim([300, 800])
    plt.show()
...
if __name__ == '__main__':
    test_dataset_preprocessor('C:\\Users\\Prestige\\Desktop\\Paolo\\UNi\\ERASMUS\\KTH\\P1\\Music Informatics\\fp_musinfo\\TEST_one_instrument', True, False)
